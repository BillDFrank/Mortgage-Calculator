...
def solve_for_unknown(data):
    ...
    if len(missing) > 1:
        raise ValueError("Please leave exactly one required field empty (for auto-calculation).")
    if len(missing) == 0:
        raise ValueError("Please leave exactly one required field empty so that it can be calculated automatically.")
    unknown = missing[0]
...
def simulate_amortization(data, computed_monthly_payment):
    ...
    total_payments = sum([row["payment"] for row in monthly_schedule]) + bank_insurances * len(monthly_schedule)
...
def generate_amortization_graph(schedule):
    ...
    traces = [
        go.Scatter(x=months, y=cum_payment, mode='lines+markers', name='Cumulative Payment'),
        go.Scatter(x=months, y=cum_extra, mode='lines+markers', name='Cumulative Extra Payment'),
        go.Scatter(x=months, y=cum_fee, mode='lines+markers', name='Cumulative Extra Fee'),
        go.Scatter(x=months, y=cum_interest, mode='lines+markers', name='Cumulative Interest'),
        go.Scatter(x=months, y=cum_principal, mode='lines+markers', name='Cumulative Principal'),
        go.Scatter(x=months, y=remaining_balance, mode='lines+markers', name='Remaining Balance')
    ]
...
def index():
    ...
    if request.method == 'POST':
        ...
        required_fields = ["house_price", "down_payment", "loan_term", "interest_rate", "monthly_payment"]
        missing_fields = []
